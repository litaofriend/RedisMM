...
<form name="form" class="form-inline" method="get" action="<?php site_url('mysql/status') ?>" >
 ...
 <select name="application_id" id="application_id" class="input-medium" style="" onchange="get_host_by_application()">
  ...
  <option value="error" <?php if($setval['level']=='error') echo "selected"; ?> >紧急</option>
   ...
   <input class="Wdate" style="width:150px;height:15px" type="text" name="stime" id="start_time>" value="<?php echo $setval['stime'] ?>" onFocus="WdatePicker({doubleCalendar:false,isShowClear:false,readOnly:false,dateFmt:'yyyy-MM-dd HH:mm'})"/>
  <input class="Wdate" style="width:150px;height:15px" type="text" name="etime" id="end_time>" value="<?php echo $setval['etime'] ?>" onFocus="WdatePicker({doubleCalendar:false,isShowClear:false,readOnly:false,startDate:'1980-05-01',dateFmt:'yyyy-MM-dd HH:mm'})"/>
