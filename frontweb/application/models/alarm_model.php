...
class Alarm_model extends CI_Model{
    ...
    function get_total_record_paging($limit,$offset){
        $this->db->select('alarm.create_time,alarm.db_type,alarm.alarm_type,alarm.alarm_value,alarm.level,alarm.message,alarm.send_mail,alarm.send_mail_status,alarm.send_mail_time,servers.host,servers.port,servers.alarm_person,application.display_name application');
        $this->db->from('alarm_history alarm');
        $this->db->where('send_mail_status','1');
