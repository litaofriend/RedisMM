...
class Servers_model extends CI_Model{
	...
	function get_record_usage_by_application($application_id){
		$this->db->where('is_delete',0);
		if ($application_id!="")
		{
			$this->db->where('application_id',$application_id);
		}
		$this->db->order_by('host,port','asc');
        $query = $this->db->get($this->table);
		if ($query->num_rows() > 0)
		{
			return $query->result_array();
    ...
    function get_total_slowquery_server(){
        ...
        $this->db->where('is_delete',0);
        $this->db->order_by('application_id,host,port','asc');
        $query = $this->db->get($this->table);
		if ($query->num_rows() > 0)
		{
			return $query->result_array();
    ...
    function get_servers_by_application($application_id){
        /*$query = $this->db->get_where($this->table, array('application_id' =>$application_id));
