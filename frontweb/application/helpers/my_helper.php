...
function get_replication_tree($array,$host='---',$port='---',$level=0){
    ...
    foreach ( ( array ) $array as $v ) {
        if($v['master_host']==$host and $v['master_port']==$port)
        {
            $host_v=$v['host'];
            $port_v=$v['port'];
...
function check_status($data){
    if($data == 1 || $data == "y" || $data == "Y"){
        return "<span class='badge badge-info'>是</span>";
    }
    else if($data == 0 || $data == "n" || $data == "N"){
...
function check_value($data){
    if($data=='---' || $data == null ){
        ...
        return "<span class='label label-important'>down</span>";
    }
    else if($data=='ok'){
        return "<span class='label label-success'>ok</span>";
    }
    else if($data=='err'){
        return "<span class='label label-important'>err</span>";
    }
...
function check_timestamp($unixtime)
{
...
function check_uptime($data){
    ...
    else if ($data >=86400*365){
    	$data=number_format(($data/(86400*365)),0)." 年 ".number_format((($data/86400)%365),0)." 天";
        return $data;
    }
}
...
function check_hits_ratio_min($data){
	if($data<=0.1){
...
function check_hits_ratio_max($data){
	if($data>=0.9){
		return "<span style='color:red'>".$data."</span>";
	}
	else {
...
function check_data_gt_value($data,$value){
        if($data>$value){
            return "<span style='color:red'>".$data."</span>";
        }
        else{
...
function check_data_lt_value($data,$value){
        if($data<$value){
            return "<span style='color:red'>".$data."</span>";
        }
        else{
